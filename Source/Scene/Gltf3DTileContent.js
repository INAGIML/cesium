...
function createBatchTable(gltf, featureTableId, buffer) {
  ...
  return {
    ...
    jsonByteLength: batchTableJsonByteLength,
    binaryByteLength: batchTableBinaryByteLength,
    batchLength: count,
  };
}

function removeNodesRecursive(gltf, nodes, callback) {
  // A node is removed if callback returns true
  // If all of a node's children have been removed, the node is removed
  var removeCount = 0;
  var nodesLength = nodes.length;
  for (var i = 0; i < nodesLength; ++i) {
    var nodeId = nodes[i];
    if (callback(gltf, nodeId)) {
...
function createFeatureTable(properties, instancesLength) {
  ...
  return {
    ...
    jsonByteLength: featureTableJsonByteLength,
    binaryByteLength: featureTableBinaryByteLength,
  };
}
...
function createInstanced3DModel(content, gltf, instancedNodeId, buffer) {
  ...
  var dataView = new DataView(i3dm);
...
function createInnerGltfContent(content, gltf, buffer) {
  ...
  if (!defined(gltf.meshes) || gltf.meshes.length === 0) {
    return undefined;
  }
